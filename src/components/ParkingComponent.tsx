...
const ParkingComponent: FC<ParkingProps> = ({
  ...
  const handlePayNow = () => {
    if (!plateNumber) {
      setPlateError("Please input license plate");
      window.scrollTo({ top: 0, behavior: "smooth" });
    } else if (duration < lot.payTime) {
      setDurationError(
        `You do not need to pay for less than ${lot.payTime} minutes`
      );
      window.scrollTo({ top: 0, behavior: "smooth" });
    } else {
      setPlateError("");
    ...
    <div className="flex flex-col w-full">
      ...
      >
        <div>
          ...
          <p className="text-red-600 text-sm">{durationError}</p>
          <div className="flex flex-col lg:flex-row justify-between lg:items-center mt-4">
          ...
          <div className="flex flex-col mt-4 items-center rounded-sm bg-[#fff2ee] p-4">
            ...
            <p className="lg:text-2xl text-xl text-[#091C62]">
              Disclaimer ― Park at Your Own Risk
            </p>
            <div className="lg:text-xl text-md text-[#091C62] mt-4">
